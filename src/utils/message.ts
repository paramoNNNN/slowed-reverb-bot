...
export const checkMessage = (ctx: CTX): boolean | RegExpMatchArray | null | undefined =>
...
type SendMessageParams = (chatId: number, text: string, options?: tt.ExtraReplyMessage) => void;
...
export const sendMessage: SendMessageParams = (chatId, text, options): void => {
  bot.telegram
    ...
    .catch((e) =>
      writeLog(
        options ? `messageId: ${options.reply_to_message_id}` : `chatId: ${chatId}`,
        "Error",
        JSON.stringify(e),
      ),
    );
